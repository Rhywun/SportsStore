...
class TetherClass extends Evented {
    ...
    position(flushChanges=true) {
        ...
        if (typeof this.options.optimizations !== 'undefined' &&
            ...
            const offsetParent =
                this.cache('target-offsetparent', () => getOffsetParent(this.target));
            const offsetPosition =
                this.cache('target-offsetparent-bounds', () => getBounds(offsetParent));
            ...
            ['Top', 'Left', 'Bottom', 'Right'].forEach(side => {
                offsetBorder[side.toLowerCase()] =
                    parseFloat(offsetParentStyle[`border${side}Width`]);
            });
        ...
        const transcribe = (_same, _pos) => {
            ...
            if (gpu !== false) {
                ...
                if (window.matchMedia) {
                    // HubSpot/tether#207
                    const retina = window.matchMedia('only screen and (min-resolution: 1.3dppx)')
                        .matches ||
